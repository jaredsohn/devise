...
module Devise
  ...
  # changed.
  mattr_accessor :explicit_set_configs
  @@explicit_set_configs = Set.new
  ...
  def self.strip_whitespace_keys=(strip_whitespace_keys)
    explicit_set_configs << :strip_whitespace_keys
  ...
  def self.email_regexp=(email_regexp)
    explicit_set_configs << :email_regexp
  ...
  def self.reconfirmable=(reconfirmable)
    explicit_set_configs << :reconfirmable
  ...
  def self.skip_session_storage=(skip_session_storage)
    explicit_set_configs << :skip_session_storage
  ...
  def self.sign_out_via=(sign_out_via)
    explicit_set_configs << :sign_out_via
  ...
  def self.warn_default_config_changed(config, current_default, new_default)
    unless config.in?(explicit_set_configs)
      warn = <<-MESSAGE.strip_heredoc
        [Devise] config.#{config} will have new default in the Devise 4.1
        To keep the current behavior please set in your devise.rb initializer file:
      ...
      MESSAGE
      ActiveSupport::Deprecation.warn(warn) unless :email_regexp.in?(explicit_set_configs)
    end
  end
